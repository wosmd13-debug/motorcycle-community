...
const seoLinks = [
  { href: "/board", label: "바이크 게시판" },
  { href: "/meetups", label: "라이딩 모집" },
  { href: "/routes", label: "바리 코스" },
  { href: "/map", label: "라이딩 지도" },
  { href: "/services", label: "주유소" },
  { href: "/gallery", label: "갤러리" },
  { href: "/marketplace", label: "중고거래" },
  { href: "/cafes", label: "바이크 카페" },
  ...
  return (
    <footer className="safe-bottom mt-auto w-full border-t-2 border-signature/30 bg-gradient-to-b from-signature-light/50 to-[var(--portal-muted)]">
      <div className="portal-container py-8">
        ...
        <nav
          className="mt-4 flex flex-wrap items-center justify-center gap-x-2 gap-y-2 text-xs text-[var(--text-muted)]"
          aria-label="주요 메뉴"
        >
          {seoLinks.map((link, index) => (
            <span key={link.href} className="inline-flex items-center gap-2">
              ...
              <Link
                href={link.href}
                className="hover:text-signature-dark hover:underline"
              >
                {link.label}
              </Link>
            </span>
